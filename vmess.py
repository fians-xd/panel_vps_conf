...
def create_vmess_account(username, expiry_days):
    try:
        # Call the shell script to create Vmess account
        result = subprocess.run(['add-ws', username, expiry_days], capture_output=True, text=True)
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python3 vmess.py <username> <expiry_days>")
        sys.exit(1)

    username = sys.argv[1]
    expiry_days = sys.argv[2]
    result_message = create_vmess_account(username, expiry_days)
